 ...
 */
void writeOutput(const std::string& message, std::ofstream* reportStream = nullptr) {
    std::cout << message;
...
bool isMediaWikiDirectory(const fs::path& path) {
    ...
    bool hasIncludesDir = fs::exists(path / "includes") && fs::is_directory(path / "includes");
    bool hasExtensionsDir = fs::exists(path / "extensions") && fs::is_directory(path / "extensions");
    bool hasSkinsDir = fs::exists(path / "skins") && fs::is_directory(path / "skins");
    
    // A valid MediaWiki installation should have at least these core components
    return hasIndexPhp && hasApiPhp && hasIncludesDir && hasExtensionsDir && hasSkinsDir;
}
...
std::string getCurrentBranch(const fs::path& repoPath) {
    std::string cmd = "cd \"" + repoPath.string() + "\" && git rev-parse --abbrev-ref HEAD 2>/dev/null";
 ...
 * 
 * @param repoPath The repository path
 * @return true if fetch succeeded, false if error or fatal messages were detected
 */
...
bool hasUncommittedChanges(const fs::path& repoPath) {
    std::string cmd = "cd \"" + repoPath.string() + "\" && git status --porcelain 2>/dev/null";
    std::string result = execCommand(cmd);
    return !result.empty();
}
 ...
 * @param errorMsg Reference to a string that will contain error output if the operation fails.
 * @return true if git pull succeeded, false if error or fatal messages were detected.
 */
...
RepoStatus checkRepository(const fs::path& repoPath, const std::string& type) {
    ...
    if (status.behindBy > 0) {
        ...
        if (g_verbose) {
            logVerbose("  [RESULT] Behind by " + std::to_string(status.behindBy) + " commit(s)");
        }
        
        // Perform git pull if conditions are met
        // Skip auto-pull in update mode (updateSingleRepo handles it)
        bool shouldPull = !g_reportOnly && !g_updateMode &&
 ...
 */
void printVerboseDirectoryHeader(const std::string& dirType, const fs::path& dirPath) {
    if (!g_verbose) return;
    std::cout << "\n" << std::string(80, '=') << "\n";
 ...
 */
std::vector<RepoStatus> scanDirectory(const fs::path& dirPath, const std::string& type) {
    ...
    for (const auto& entry : fs::directory_iterator(dirPath)) {
        ...
        futures.emplace_back(std::async(std::launch::async, [path = entry.path(), type]() {
 ...
 * @param reportStream Optional output file stream for the report
 */
void printResults(const std::vector<RepoStatus>& results, std::ofstream* reportStream = nullptr) {
    if (results.empty()) {
        return;
    }
    ...
    oss << std::left << std::setw(30) << "Name" 
              ...
              << std::setw(15) << "Branch"
              << std::setw(10) << "Behind"
              << std::setw(14) << "Uncommitted"
              << "Status\n";
    oss << std::string(100, '-') << "\n";
    
    for (const auto& status : results) {
        oss << std::left << std::setw(30) << status.name
                  << std::setw(12) << status.type
                  << std::setw(15) << (status.currentBranch.empty() ? "N/A" : status.currentBranch);
        ...
        if (!status.isRepo) {
            ...
            oss << std::setw(10) << "0"
                      << std::setw(14) << (status.hadUncommittedChanges ? "Yes" : "No");
            if (status.hadUncommittedChanges) {
            ...
            oss << std::setw(10) << status.behindBy
                      << std::setw(14) << (status.hadUncommittedChanges ? "Yes" : "No")
 ...
 * @param reportStream Optional output file stream for the report
 */
void printResultsSection(const std::string& title, const std::vector<RepoStatus>& results, std::ofstream* reportStream = nullptr) {
    std::ostringstream oss;
    oss << "\n" << title << ":\n";
    writeOutput(oss.str(), reportStream);
 ...
 */
int updateSingleRepo(const fs::path& basePath, const std::string& type, const std::string& name) {
    fs::path repoPath;
    ...
    if (type == "core") {
        ...
        std::cerr << "Error: Invalid type '" << type << "'. Must be 'core', 'extension', or 'skin'.\n";
        return 1;
    }
    
    if (!fs::exists(repoPath)) {
        std::cerr << "Error: " << displayName << " not found at: " << repoPath.string() << "\n";
        return 1;
    }
    
    if (!fs::is_directory(repoPath)) {
        std::cerr << "Error: Path exists but is not a directory: " << repoPath.string() << "\n";
        return 1;
    }
    ...
    std::cout << "  Uncommitted changes: " << (status.hadUncommittedChanges ? "Yes" : "No") << "\n";
    std::cout << "  Commits behind: " << (status.behindBy >= 0 ? std::to_string(status.behindBy) : "Unknown") << "\n";
    ...
    // Prompt user
    std::ostringstream prompt;
    prompt << "\nPull " << status.behindBy << " commit" << (status.behindBy > 1 ? "s" : "") << "?";
    if (status.hadUncommittedChanges) {
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (arg == "-v" || arg == "--verbose") {
            ...
            if (g_updateType == "core") {
                ...
                std::cerr << "Error: --update " << g_updateType << " requires NAME argument\n";
            ...
            std::cout << "  --report-only      Only report status, don't pull updates\n";
            ...
            std::cout << "                     TYPE must be 'core', 'extension', or 'skin'\n";
            std::cout << "                     NAME required for extension/skin\n";
            std::cout << "                     Examples:\n";
            std::cout << "                       --update core\n";
            std::cout << "                       --update extension WikimediaEvents\n";
            ...
            std::cout << "      A warning will be shown if uncommitted changes exist.\n\n";
    ...
    if (!isMediaWikiDirectory(basePath)) {
        std::cerr << "Error: Directory does not appear to be a MediaWiki installation.\n";
        std::cerr << "Expected files/directories not found (index.php, api.php, includes/, extensions/, skins/).\n";
        return 1;
    }
    ...
    std::vector<RepoStatus> extensionResults = scanDirectory(extensionsPath, "extension");
    ...
    int totalRepos = static_cast<int>(coreResults.size() + extensionResults.size() + skinResults.size());
    int upToDate = coreStats.upToDate + extensionStats.upToDate + skinStats.upToDate;
    int hasUpdates = coreStats.hasUpdates + extensionStats.hasUpdates + skinStats.hasUpdates;
